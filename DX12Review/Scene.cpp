...
void CScene::CreateGraphicsRootSignature(ID3D12Device* pd3dDevice)
{
	...
	hr = pd3dDevice->CreateRootSignature(0, pd3dSignatureBlob->GetBufferPointer(), pd3dSignatureBlob->GetBufferSize(), IID_PPV_ARGS(m_pd3dGraphicsRootSignature.GetAddressOf()));
	if (FAILED(hr))
	{
...
void CScene::CreateGraphicsPipelineState(ID3D12Device* pd3dDevice)
{
	...
	HRESULT hr = D3DCompileFromFile(L"Shaders.hlsl", NULL, NULL, "VSMain", "vs_5_1", nCompileFlags, 0, &pd3dVertexShaderBlob, NULL);
	if (FAILED(hr))
	{
		OutputDebugString(L"VS Compile Failed\n");
	}
	hr = D3DCompileFromFile(L"Shaders.hlsl", NULL, NULL, "PSMain", "ps_5_1", nCompileFlags, 0, &pd3dPixelShaderBlob, NULL);
	if (FAILED(hr))
	{
	...
	hr = pd3dDevice->CreateGraphicsPipelineState(&d3dPipelineStateDesc, IID_PPV_ARGS(m_pd3dPipelineState.GetAddressOf()));
	if (FAILED(hr))
	{
		OutputDebugString(L"CreateGrahpicsPipelineState Failed\n");
	}
