	...
	:CMesh(pd3dDevice, pd3dCommandList)
{
	...
	int i = 0;
	//전면 왼쪽 위 0
	m_pVertices[i++] = CDiffusedVertex(XMFLOAT3(-fx, +fy, -fz), RANDOM_COLOR);
	//전면 오른쪽 위 1
	m_pVertices[i++] = CDiffusedVertex(XMFLOAT3(+fx, +fy, -fz), RANDOM_COLOR);
	//후면 오른쪽 위 2
	m_pVertices[i++] = CDiffusedVertex(XMFLOAT3(+fx, +fy, +fz), RANDOM_COLOR);
	...
	//앞면 위쪽 삼각형
	m_pnIndices[0] = 0, m_pnIndices[1] = 1, m_pnIndices[2] = 5;
	//앞면 아래쪽 삼각형
	m_pnIndices[3] = 0, m_pnIndices[4] = 5, m_pnIndices[5] = 4;
	//윗면 위쪽 삼각형
	m_pnIndices[6] = 3, m_pnIndices[7] = 2, m_pnIndices[8] = 1;
	//윗면 아래쪽 삼각형
	m_pnIndices[9] = 3, m_pnIndices[10] = 1, m_pnIndices[11] = 0;
	//뒷면 위쪽 삼각형
	m_pnIndices[12] = 7, m_pnIndices[13] = 2, m_pnIndices[14] = 3;
	//뒷면 아래쪽 삼각형
	m_pnIndices[15] = 7, m_pnIndices[16] = 6, m_pnIndices[17] = 2;
	//아래면 위쪽 삼각형
	m_pnIndices[18] = 4, m_pnIndices[19] = 5, m_pnIndices[20] = 6;
	//아래면 아래쪽 삼각형
	m_pnIndices[21] = 4, m_pnIndices[22] = 6, m_pnIndices[23] = 7;
	//옆면(l) 위쪽 삼각형
	m_pnIndices[24] = 3, m_pnIndices[25] = 0, m_pnIndices[26] = 4;
	//옆면(l) 아래쪽 삼각형
	m_pnIndices[27] = 3, m_pnIndices[28] = 4, m_pnIndices[29] = 7;
	//옆면(r) 위쪽 삼각형
	m_pnIndices[30] = 1, m_pnIndices[31] = 2, m_pnIndices[32] = 6;
	//옆면(r) 아래쪽 삼각형
	m_pnIndices[33] = 1, m_pnIndices[34] = 6, m_pnIndices[35] = 5;
	...
	CDiffusedVertex m_pVertices[36];
	int i = 0;
	...
	m_pd3dVertexBuffer = ::CreateBufferResource(pd3dDevice, pd3dCommandList, m_pVertices, m_nVertices * m_nStride, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, &m_pd3dVertexUploadBuffer);
	...
	: CMesh(pd3dDevice, pd3dCommandList)
{
	...
	첫 번째 원기둥은 위아래로 두 개의 정점이 필요하므로 원기둥의 표면의 사각형들을 표현하기 위하여 필요한 정점의 개수는 {(nSlices * (nStacks–1))}이다.
	그런데 구의 위와 아래(구가 지구라고 가정할 때 남극과 북극)를 자르면 원기둥이 아니라 원뿔이 되므로 이 원뿔을 표현하기 위하여 2개의 정점이 더 필요하다.
	...
	/*원기둥의 표면에 존재하는 사각형의 개수는 {nSlices * (nStacks-2)}이고 사각형은 2개의 삼각형으로 구성되므로	삼각형 리스트일 때 필요한 인덱스의 개수는 {nSlices * (nStacks-2) * 2 * 3}이다.
	그리고 구의 위아래 원뿔의 표면에 존재하는 삼각형의 개수는 nSlices개이므로 구의 위아래 원뿔을 표현하기 위한 인덱스의 개수는 {(nSlices * 3) * 2}이다.
	그러므로 구의 표면을 삼각형 리스트로 표현하기 위하여 필요한 인덱스의 개수는 {(nSlices * 3) * 2 + (nSlices * (nStacks - 2) * 3 * 2)}이다*/
