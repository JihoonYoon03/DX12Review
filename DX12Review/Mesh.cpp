...
float CHeightMapImage::GetHeight(float fx, float fz)
{
	...
	if (bRightToLeft)
	{
		//삼각형이 오른쪽에서 왼쪽으로 나열되는 경우이다
		//z = x의 형태로 삼각형 2개를 나누는 대각선이 그려진다. /와 같은 모양
		if (fzPercent >= fxPercent)
			//fz% > fx%인 경우, 좌표값이 위쪽 삼각형을 가리킨다.
			//이 때, 우측 하단 정점의 높이는 TopRight -> TopLeft에서의 높이 증가율을 BottomLeft에 더해 구한다.
			//결과적으로 평평한 사각형이 나오게 된다.
			fBottomRight = fBottomLeft + (fTopRight - fTopLeft);
		else
			//위와 같은 원리로 TopLeft를 구한다.
			fTopLeft = fTopRight + (fBottomLeft - fBottomRight);
	}
	else
	{
		//삼각형이 왼쪽에서 오른쪽으로 나열되는 경우이다.
		//z = x의 역수 형태로 삼각형 2개를 나누는 대각선이 그려지는 줄이다. \와 같은 모양
	...
	: CMesh(pd3dDevice, pd3dCommandList)
{
	...
	'사각형 줄'의 개수는 (nLength - 1)이고 한 줄에서 (nWidth * 2)개의 인덱스를 갖는다.
	그리고 줄이 바뀔 때마다 인덱스를 하나 추가하므로 (nLength-1) - 1(첫줄 제외) 개의 인덱스가 추가로 필요하다.*/
...
XMFLOAT4 CHeightMapGridMesh::OnGetColor(int x, int z, void* pContext)
{
	...
	/*정점 (x, z)에서 조명이 반사되는 양(비율)은 정점 (x, z)의 법선 벡터와 조명의 방향 벡터의 내적(cos)과 인접한 3개의 정점 (x+1, z), (x, z+1), (x+1, z+1)의 법선 벡터와 조명의 방향 벡터의 내적을 평균하여 구한다.
