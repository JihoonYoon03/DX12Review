...
{
	//if (m_pMesh) m_pMesh->Release();
	m_vpMeshes.at(nIndex) = pMesh;
	//if (m_pMesh) m_pMesh->AddRef();
}

void CGameObject::SetMesh(int nIndex, std::shared_ptr<CMesh>&& pMesh)
{
	m_vpMeshes.at(nIndex) = std::move(pMesh);
}

void CGameObject::ReleaseUploadBuffers()
{
	//정점 버퍼를 위한 업로드 버퍼를 소멸시킨다.
	for (std::shared_ptr<CMesh>& mesh : m_vpMeshes)
	{
	...
	: CGameObject(0)
{
	...
	m_vpMeshes.resize(cxBlocks * czBlocks);
	...
	for (int z = 0, zStart = 0; z < czBlocks; z++)
	{
		for (int x = 0, xStart = 0; x < cxBlocks; x++)
		{
			...
			pHeightMapGridMesh = std::make_shared<CHeightMapGridMesh>(CHeightMapGridMesh(pd3dDevice, pd3dCommandList, xStart, zStart, nBlockWidth, nBlockLength, xmf3Scale, xmf4Color, m_pHeightMapImage));
