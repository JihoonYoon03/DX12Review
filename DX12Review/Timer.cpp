...
void CGameTimer::Tick(float fLockFPS)
{
	...
	//이 함수를 호출한 이후 경과한 시간 계산
	float fTimeElapsed = (m_nCurrentTime - m_nLastTime) * m_fTimeScale;

	if (fLockFPS > 0.0f)
	{
		//이 함수의 파라미터가 0보다 크면 이 시간만큼 호출한 함수를 기다리게 함
	...
	//프레임 처리 시간 차이 < 1초이면, 현재 프레임 처리 시간을 m_fFrameTime[0]에 저장
	...
	//초당 프레임 수를 1 증가시키고 현재 프레임 처리 시간을 누적해 저장
	m_nFramesPerSecond++;
	...
	//누적된 프레임 처리 시간의 평균을 구해 프레임 처리 시간을 구한다.
	m_fTimeElapsed = 0.0f;
